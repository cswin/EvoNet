...
parser.add_argument('--src_dir_clean', dest='src_dir_clean', default='./data/TrainCT_clean', help='dir of data')
parser.add_argument('--src_dir_noisy', dest='src_dir_noisy', default='./data/TrainCT_noisy', help='dir of data')
...
parser.add_argument('--from_file', dest='from_file', default="./data/img_clean_ct.npy", help='get pic from file')
...
def generate_patches(isDebug=False):
    ...
    count = 0
    filepaths_clean = glob.glob(args.src_dir_clean + '/*.bmp')
    filepaths_noisy = glob.glob(args.src_dir_noisy + '/*.bmp')
    if isDebug:
        filepaths_clean = filepaths_clean[:10]
    print "number of training data %d" % len(filepaths_clean)
    
   # scales = [1, 0.9, 0.8, 0.7]
    scales = [1]
    # calculate the number of patches
    for i in range(len(filepaths_clean)):
        img = Image.open(filepaths_clean[i]).convert('L')  # convert RGB to gray
...
#   print "total patches = %d , batch size = %d, total batches = %d" % \
 #       (numPatches, args.bat_size, numPatches / args.bat_size)
    ...
    noisydata=np.zeros((numPatches, args.pat_size, args.pat_size, 1), dtype="uint8")
    count = 0
    # generate patches
    for i in xrange(len(filepaths_clean)):
        img = Image.open(filepaths_clean[i]).convert('L')
        img_noisy=Image.open(filepaths_noisy[i]).convert('L')
        for s in xrange(len(scales)):
            ...
            img_s_n = img_noisy.resize(newsize, resample=PIL.Image.BICUBIC)
            img_s_n = np.reshape(np.array(img_s_n, dtype="uint8"),
                               (img_s_n.size[0], img_s_n.size[1], 1))  # extend one dimension.
...
#                                                                   random.randint(0, 7))
#                        noisydata[count, :, :, :] = data_augmentation(img_s_n[x:x + args.pat_size, y:y + args.pat_size, :], \
#                                                                   random.randint(0, 7))
                        noisydata[count, :, :, :] = img_s_n[x:x + args.pat_size, y:y + args.pat_size, :]
                        count += 1
    # pad the batch
    if count < numPatches:
        to_pad = numPatches - count
        inputs[-to_pad:, :, :, :] = inputs[:to_pad, :, :, :]
        noisydata[-to_pad:, :, :, :] = noisydata[:to_pad, :, :, :]
    
    if not os.path.exists(args.save_dir):
        os.mkdir(args.save_dir)
    np.save(os.path.join(args.save_dir, "img_clean_ct_clean"), inputs)
    np.save(os.path.join(args.save_dir, "img_clean_ct_noisy"), noisydata)
